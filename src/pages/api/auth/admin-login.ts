...
export const POST: APIRoute = async (context) => {
  try {
    ...
    if (!email || !password) {
      return context.redirect(`/admin/login?error=${encodeURIComponent('Email y contraseña requeridos')}`);
    }
    ...
    if (error || !data.session) {
      return context.redirect(`/admin/login?error=${encodeURIComponent('Email o contraseña incorrectos')}`);
    }
