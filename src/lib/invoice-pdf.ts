...
/**
 * Genera un PDF de factura usando pdf-lib (puro JS, sin dependencias nativas)
 */
export async function generateInvoicePDF(data: InvoiceData): Promise<Buffer> {
    ...
    cy -= 14;
    page.drawText(data.customerName, { x: clientX, y: cy, size: 8, font: helvetica, color: grayText });
    cy -= 12;
    ...
    for (const item of data.items) {
        ...
        // Sanitize: replace non-latin characters that Helvetica can't render
    ...
    const totalText = `${data.total.toFixed(2)} EUR`;
    const totalW = helveticaBold.widthOfTextAtSize(totalText, 14);
    page.drawText(totalText, { x: col4 - totalW, y, size: 14, font: helveticaBold, color: darkBlue });
    ...
    const footer2 = 'Gracias por tu compra. Este documento sirve como comprobante de tu pedido.';
